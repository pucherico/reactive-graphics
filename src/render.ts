import { Subscription, Subject, Observable, interval, timer, range, of, from, fromEvent, merge, zip, animationFrameScheduler, BehaviorSubject, EMPTY,
    share, filter, first, mapTo, map, defaultIfEmpty, isEmpty, mergeMap, switchMap, takeUntil, auditTime, bufferTime,
    tap, scan, withLatestFrom, distinctUntilChanged, toArray, take
  } from 'rxjs';
  ...
  export interface Drawable { draw(context: CanvasRenderingContext2D): void }
  ...
  class Layer<T extends Drawable> {
    ...
    bringToTop(graph: T) {
      this.graphics.push(...this.graphics.splice(this.graphics.indexOf(graph), 1));
    }
  
    sendToBack(graph: T) {
      this.graphics.unshift(...this.graphics.splice(this.graphics.indexOf(graph), 1));
    }
    ...
    set geomCoefficients([a, b, c, d, e, f]: [number, number, number, number, number, number]) {
  ...
  class PaceFrameAdapter {
    ...
    private readonly paceSubject: BehaviorSubject<number> = new BehaviorSubject(1);
  ...
  class CanSleepFrameAdapter {
    ...
    public readonly frame$: Observable<number>;
    private readonly sleepSubject: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(false);
  ...
  class FrameManager {
    ...
    constructor(parentFrame: Observable<number> | null = null) {
      const originalFrame = parentFrame ?? interval(0, animationFrameScheduler).pipe(
  ...
  export class RenderManager<L extends Layer<Drawable> = Layer<Drawable>> {
    ...
    removeGraphic(graph: Drawable): number {
      ...
      if (layer === undefined) { throw Error(`Can not remove unknown graph.`); }
    ...
    bringToTop(graph: Drawable) {
      const layer = this.layerMap.get(graph);
      if (layer === undefined) { throw Error(`Can not bring to top unknown graph.`); }
      this.layers[layer].bringToTop(graph);
    }
  
    sendToBack(graph: Drawable) {
      const layer = this.layerMap.get(graph);
      if (layer === undefined) { throw Error(`Can not send to back unknown graph.`); }
    ...
    layerOfGraph(graph: Drawable): number {
      const layer = this.layerMap.get(graph);
      if (layer === undefined) { throw Error(`Can not find graph layer.`); }
      return layer;
    }
    ...
    rectInLayer(rect: Rect, layer: number): Rect {
      if (layer < 0 || layer >= this.layers.length) { throw Error(`Invalid layer ${layer}.`); }
      return new Rect(this.layers[layer].inverseTransform(rect.from), this.layers[layer].inverseTransform(rect.to));
    }
  
    rectFromLayer(rect: Rect, layer: number): Rect {
      if (layer < 0 || layer >= this.layers.length) { throw Error(`Invalid layer ${layer}.`); }
      return new Rect(this.layers[layer].transform(rect.from), this.layers[layer].transform(rect.to));
    }
    ...
    layerGeomCoefficients(layer: number): [number, number, number, number, number, number] {
      if (layer < 0 || layer >= this.layers.length) { throw Error(`Invalid layer ${layer}.`); }
      return this.layers[layer].geomCoefficients;
    }
  
    setLayerGeomCoefficients(layer: number, [a, b, c, d, e, f]: [number, number, number, number, number, number]) {
      if (layer < 0 || layer >= this.layers.length) { throw Error(`Invalid layer ${layer}.`); }
  ...
  export interface Contact { graph: GraphObject; vector: Point }
  
  interface LayeredContacts { layer: number; contacts: Observable<Contact> }
  ...
  export class PointerInput {
    ...
    public readonly drag$:  Observable<{origin: Point; contact: Contact | null; point: Point}>;
    private readonly enableInputSubject: BehaviorSubject<boolean> = new BehaviorSubject<boolean>(true);
    ...
    constructor(canvas: HTMLCanvasElement, private detector: ContactDetection) {
      ...
      this.start$ = merge(
        fromEvent<MouseEvent>(canvas, 'mousedown').pipe(map((event: MouseEvent) => ({x: event.offsetX, y: event.offsetY}))),
        fromEvent<TouchEvent>(canvas, 'touchstart').pipe(map((event: TouchEvent) => this.touchEventToPoint(event, canvas))),
      ).pipe(
      ...
      this.move$ = merge(
        ...
        fromEvent<TouchEvent>(canvas, 'touchmove').pipe(map((event: TouchEvent) => this.touchEventToPoint(event, canvas)))
      ).pipe(
        withLatestFrom(inputEnabled),
        filter(([_point, enabled]) => enabled),
        map(([point, _enabled]) => point)
      ...
      this.end$ = merge(
        fromEvent<MouseEvent>(canvas, 'mouseup').pipe(map((event: MouseEvent) => ({x: event.offsetX, y: event.offsetY}))),
        fromEvent<TouchEvent>(canvas, 'touchend').pipe(map((event: TouchEvent) => this.touchEventToPoint(event, canvas)))
      ).pipe(
        withLatestFrom(inputEnabled),
        filter(([_point, enabled]) => enabled),
        map(([point, _enabled]) => point)
      ...
      this.click$ = this.start$.pipe(
        switchMap((point: Point) => merge(
          timer(CLICK_TIMEOUT).pipe(map(() => ({ timeout: true, init: point, end: null }))),
          this.end$.pipe(map(endPoint => ({ timeout: false, init: point, end: endPoint }))),
          ).pipe(first())
        ),
        filter(({ timeout, init, end }) => !timeout && squareDistance(init, end as Point) < CLICK_EPSILON),
      ...
      this.doubleClick$ = this.click$.pipe(
        switchMap((_: Point) =>
          merge(
            timer(CLICK_TIMEOUT).pipe(mapTo(null)),
            this.end$
          ).pipe(first())
        ),
      ...
      this.oneClick$ = this.click$.pipe(
        ...
        scan(([accTipo, _accPoint], [tipo, point]) => [accTipo === 2 && tipo === 1 ? 3 : tipo, point] as [number, Point]),
    ...
    protected touchEventToPoint(event: TouchEvent, canvas: HTMLCanvasElement): Point {
    ...
    filterPointWithoutContact(): (source: Observable<Point>) => Observable<Point> {
      return (source: Observable<Point>) => {
        return source.pipe(
          mergeMap(point => this.detector.checkContact(point).pipe(
            isEmpty(),
            filter(empty => empty),
            mapTo(point)
          ))
     ...
     */
    filterPointWithoutContactAboveLayer(layer: number): (source: Observable<Point>) => Observable<Point> {
      return (source: Observable<Point>) => {
        return source.pipe(
          mergeMap(point => this.detector.checkContactAboveLayer(point, layer).pipe(
            isEmpty(),
            filter(empty => empty),
            mapTo(point)
          ))
    ...
    filterPointWithContact(graph: GraphObject): (source: Observable<Point>) => Observable<{ point: Point; contact: Contact }> {
      return (source: Observable<Point>) => source.pipe(
        this.pointContact(),
     ...
     */
    filterPointWithContacts(): (source: Observable<Point>) => Observable<{ point: Point; contacts: Contact[] }> {
      return (source: Observable<Point>) => {
        return source.pipe(
          mergeMap(point => this.detector.checkContact(point).pipe(
            toArray(),
     ...
     * Junto al punto, se devuelve información de los contactos
     */
    filterPointWithTopContacts(): (source: Observable<Point>) => Observable<{ point: Point; contacts: Contact[] }> {
      return (source: Observable<Point>) => {
        return source.pipe(
          mergeMap(point => of(point).pipe(
            this.topContact(),
            toArray(),
            filter(contacts => contacts.length > 0),
            map(contacts => ({ point, contacts }))
    ...
    pointContact(): (source: Observable<Point>) => Observable<{ point: Point; contact: Contact }> {
      return (source: Observable<Point>) => {
        return source.pipe(
          mergeMap(point => this.detector.checkContact(point).pipe(map(contact => ({ point, contact }))))
        );
      };
    }
    ...
    topContact(): (source: Observable<Point>) => Observable<Contact> {
      return (source: Observable<Point>) => {
        return source.pipe(
          mergeMap(point => this.detector.checkLayerContacts(point).pipe(
            mergeMap(({ layer, contacts }) => contacts.pipe(
              isEmpty(),
              filter(empty => !empty),
    ...
    pointTopContact(): (source: Observable<Point>) => Observable<{ point: Point; contact: Contact }> {
      return (source: Observable<Point>) => {
        return source.pipe(
          mergeMap(point => this.detector.checkLayerContacts(point).pipe(
            mergeMap(({ layer, contacts }) => contacts.pipe(
  ...
  class CollisionLayer extends Layer<GraphObject> {
      ...
      return from(this.objects).pipe(
        ...
        mergeMap(obj =>
          zip( 
            ...
            map(([obj, _inside]: [GraphObject, boolean]) => ({graph: obj, vector: vector(obj.position, layerPoint)}))
          )
        )
      );
    }
    ...
    checkCollision(graph: GraphObject): Observable<GraphObject> {
      ...
      return range(0, objects.length).pipe(
        ...
        // PHASE II: thorough removing with remaining objects (by looking into every rect in the boundaries)
        mergeMap(g => from(g.getBoundaries()).pipe(
          mergeMap((r: Rect) => from(graph.getBoundaries()).pipe(
    ...
    checkCollisions(): Observable<[GraphObject, GraphObject]> {
      ...
      return range(0, n).pipe(
        ...
        // PHASE I: fast-easy preliminary removing
        filter(([graph1, graph2]: [GraphObject, GraphObject]) => graph1.getBoundingRect().intersect(graph2.getBoundingRect())),
        // PHASE II: thorough removing with remaining objects (by looking into every rect in the boundaries)
        mergeMap(([graph1, graph2]: [GraphObject, GraphObject]) =>
          from(graph1.getBoundaries()).pipe(
            mergeMap((rect1: Rect) => from(graph2.getBoundaries()).pipe(
  ...
  export class GraphEngine extends RenderManager<CollisionLayer> implements ContactDetection, CollisionDetection {
    ...
    constructor(public readonly canvas: HTMLCanvasElement) {
      ...
      this.collision$ = this.frame$.pipe(
        ...
        switchMap(_frametime => from(this.layers).pipe(
    ...
    checkContactInLayer(point: Point, layer: number): Observable<Contact> {
      return this.layerEnabled(layer) ? this.layers[layer].checkContact(point) : EMPTY;
    }
  
    checkContactAboveLayer(point: Point, layer: number): Observable<Contact> {
      const layerPairs = this.layers
        .map((layer, index) => ({ layer, enabled: this.layerEnabled(index) }));
    ...
    checkLayerContacts(point: Point): Observable<LayeredContacts> {
      return range(0, this.layers.length).pipe(
        map(layerIndex => ({ layer: layerIndex, contacts: this.checkContactInLayer(point, layerIndex) }))
      );
    }
  
    checkCollision(graph: GraphObject): Observable<GraphObject> {
      const layer = this.layerMap.get(graph);
      if (layer === undefined) { throw Error(`Can not find unknown graph.`); }
